...
Set rs = db.Execute("SELECT * FROM ext_cal_context ORDER BY name")
do while not rs.EOF
	...
	context_list = context_list & ">" & rs("name") & VbCrLf
	rs.movenext
loop
rs.Close
...
query = "SELECT * FROM ext_cal_events WHERE event_context=" & event_context _
...
do while not rs.EOF
	events(Day(rs("event_start"))) = events(Day(rs("event_start"))) _
		& "<br><a href='ext_cal_detail.asp?event_id=" & rs("event_id") _
		& "'>" & rs("event_title") & "</a>"
...
' color weekdays white, weekends grey and today yellow
			...
			cal = cal & "<font face='arial'><font size=2>" _
				& "<a href='ext_cal_edit.asp?" _
				& "action=add" _
...
<a href="ext_cal_print.asp?event_context=<%=event_context%>&month=<%=m%>&year=<%=y%>" target="_top">
...
response.write "<td><font face='arial' size=2><b><a href='ext_cal.asp?" _
	& "event_context=" & event_context _
	& "&month=" & mPrev _
	& "&year=" & yPrev & "'>&lt;" _
...
response.write "<form method='post' action='ext_cal.asp'>" _
